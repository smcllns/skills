...
def token_capture_status(harness: str, session_id: str, transcript_path: str) -> str:
    """"measured" when a harness adapter can read tokens, else "unavailable".

    Single source of truth for D5: with no supported session (e.g. Pi/WhatsApp)
    token cost is unavailable, never $0.
    """
    if harness == "claude":
        return "measured" if transcript_path and Path(transcript_path).is_file() else "unavailable"
    if harness == "codex" and session_id:
        sessions_root = Path.home() / ".codex" / "sessions"
        return "measured" if cost_mod._codex_index(sessions_root).get(session_id) else "unavailable"
    return "unavailable"
...
def cmd_preflight(args: argparse.Namespace) -> int:
    """Confirm token capture before research and emit the cost_capture receipt.

    cost_capture passes either way — the gate proves cost was handled, not that a
    session exists. With no session the reason records "unavailable" so the report
    renders unavailable (D5), never $0.
    """
    harness = detect_harness()
    status = token_capture_status(harness["harness"], harness["session_id"], harness["transcript_path"])
    reason = (
        "token session detected; LLM cost will be measured"
        if status == "measured"
        else "no token session (e.g. Pi/WhatsApp) — LLM cost will render unavailable, never $0"
    )
    if args.ledger:
        from core.gate_ledger import append_receipt

        append_receipt(args.ledger, "cost_capture", "pass", reason=reason)
    print(json.dumps({"token_status": status, "harness": harness["harness"], "reason": reason}))
    return 0
...
def build_cost(
    *,
    ...
    token_status = token_capture_status(harness, session_id, transcript_path)
    scopes = cost_mod.TokenScopes()
    if token_status == "measured":
        scopes = cost_mod.claude_collect(transcript_path, window) if harness == "claude" else cost_mod.codex_collect(session_id, window)
...
def _aggregate_source(parts: list[VendorLineItem]) -> VendorLineItem:
    ...
    if len({part.unit_rate for part in parts}) == 1:
        detail = base.detail
        if len(parts) > 1:
            units = cost / base.unit_rate if base.unit_rate else calls
            if base.unit == "per post read":
                detail = f"aggregated across {len(parts)} runs; {calls} call(s) = {units:,.0f} reads × ${base.unit_rate:g} = ${cost:,.2f}"
            elif base.unit_rate:
                detail = f"aggregated across {len(parts)} runs; {calls} call(s) × ${base.unit_rate:g} {base.unit} = ${cost:,.2f}"
            else:
                detail = f"aggregated across {len(parts)} runs"
        return replace(base, calls=calls, cost_usd=cost, detail=detail)
...
def main(argv: list[str] | None = None) -> int:
    ...
    preflight = sub.add_parser("preflight", help="check token capture before research; emit cost_capture receipt")
