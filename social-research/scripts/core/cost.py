...
def render_appendix_d(cost: dict[str, Any]) -> str:
    ...
    # Distinguish a *measured* token cost (possibly genuinely $0) from one that was
    # never captured (no agent harness/session — e.g. Pi/WhatsApp). The latter must
    # render "unavailable", never "$0.00", or the report claims a false free run.
    token_unavailable = cost.get("token_status", "measured") == "unavailable"

    vendor_rows = "".join(
        "<tr>"
        f"<td>{html.escape(item['source'])}{_detail_span(item.get('detail'))}</td>"
    ...
    if token_unavailable:
        token_rows = (
            "<tr><td>LLM tokens — <strong>unavailable</strong>"
            "<br><span class=\"meta\">no measured agent session (e.g. Pi/WhatsApp harness "
            "with no transcript); token cost was not captured — this is not $0</span></td>"
            "<td class=\"num\">—</td><td class=\"num\">—</td><td class=\"num\">—</td>"
            "<td class=\"num\">—</td><td class=\"num\">unavailable</td></tr>"
        )
    else:
        token_rows = "".join(
            "<tr>"
    ...
    token_subtotal_display = "unavailable" if token_unavailable else _usd(token_total)
    if token_unavailable:
        total_line = (
            f"<strong>Total effective cost / report: {_usd(vendor_total)} + LLM tokens unavailable</strong> "
            f"({_usd(vendor_total)} data-vendor measured; LLM token session not detected, "
            "so the total is a floor, not the full cost)."
        )
    else:
        total_line = (
            f"<strong>Total effective cost / report: {_usd(total)}</strong> "
            f"({_usd(vendor_total)} data-vendor + {_usd(token_total)} LLM tokens)."
        )
  ...
  <div class="details-body">
    ...
    <table>
      ...
      <tbody>
        {token_rows}
        <tr><td><strong>LLM subtotal</strong></td><td class="num"></td><td class="num"></td><td class="num"></td><td class="num"></td><td class="num"><strong>{token_subtotal_display}</strong></td></tr>
      </tbody>
    </table>
    <p class="note">{total_line}</p>
...
def _detail_span(detail: str | None) -> str:
    """Render a vendor line item's amortization/detail as a meta sub-line so the
    displayed numbers reconcile at a glance (e.g. X: calls × reads/call × rate)."""
    if not detail:
        return ""
    return f"<br><span class=\"meta\">{html.escape(detail)}</span>"
