...
- **Image lightbox** — every skin ships a shared, dependency-free lightbox: any
  `img.photo` (top-pick / near-miss screenshots) is click- and keyboard-activatable
  to view full-screen, with the image's `alt` as the caption; Esc or a backdrop
  click closes. It's automatic — authors do nothing beyond using `img.photo` (as
  the templates already do). So write **meaningful `alt` text**: it doubles as the
  lightbox caption. Don't hand-roll a second viewer.
...
- **Appendix D** present and open when a cost record was produced: generated by
  ...
  When there is **no token session** (e.g. Pi/WhatsApp), the LLM rows must read
  **"unavailable"**, never `$0.00` — `cost_report.py` emits this automatically.
...
- Run the release gate: `python3 scripts/validate_html_report.py <file> --ledger
  "$RUN/gate_ledger.jsonl"`; it must pass. It fails closed if any required gate
  receipt is missing or not `pass` (see SKILL.md → Release gate ledger), rejects a
  map fallback whose reason isn't sanctioned ("speed" is not), and rejects
  SVG/placeholder photos when photo creds are present — unless a `draft_approved`
  receipt is recorded. Use `--skip-gates` only for a standalone structural check.
